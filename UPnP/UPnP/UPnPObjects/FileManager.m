...
- (void)buildFileList
{
    ...
    {
        NSString *fullPath = [[NSFileManager pathToSharedDocuments] stringByAppendingString:file];
    ...
    _baseDirectory = [[StorageFolder alloc] initObjectFromDictionary:
                      [NSDictionary dictionaryWithObjectsAndKeys:
                       @"parentId", @"-1",
                       @"isSearchable", [NSNumber numberWithBool:searchSupported],
                       @"isRestricted", [NSNumber numberWithBool:YES],
                       @"childCount", [NSNumber numberWithInt:directories],
                       @"title", @"Media", 
                       @"storageUsed", [NSNumber numberWithLong:[NSFileManager folderSize:[NSFileManager pathToSharedDocuments]]], 
                       @"relativePath", [NSFileManager pathToSharedDocuments], nil]];
...
- (StorageFolder *)folderWithObjectID:(NSString *)objectID
{
    ...
    for (StorageFolder *subfolder in _baseDirectory.subfolders) 
    {
                      ...
                      totalCount:(int *)totalCount;
{
    ...
    *totalCount = 0;
    ...
    if (!folder) 
    {
        DDLogError(@"Received request for invalid object ID %i", objectID);
    ...
    if (showChildren) 
    {
        DDLogInfo(@"Received request for children of object %i", objectID);
    ...
    {
        DDLogInfo(@"Received request for metadata of object %i", objectID);
